...
<div class="px-6">
    <div class="flex flex-col py-4">
        <span class="text-lg font-bold text-black dark:text-white">Data Alternative</span>
        <span class="text-sm text-black dark:text-white">Accessing alternative data sources for comprehensive analysis.</span>
    </div>
    <div>
            ...
            <tbody class="text-sm font-thin text-black dark:text-white">
                <?php foreach ($Alternatif as $index => $row) : ?>
                    <tr>
                        ...
                        <td class="border border-gray-200 dark:border-gray-700"><?php echo $row['kode_alternatif']; ?></td>
                        <td class="border border-gray-200 dark:border-gray-700"><?php echo $row['nama_alternatif']; ?></td>
                        <?php if (session()->get('id_user_level') == 2) : ?>
                            <td class="border border-gray-200 dark:border-gray-700">
                                <div class="flex items-center space-x-2 text-lg">
                                    <button onclick="showEditAlternative(<?php echo $row['id_alternatif']; ?>)" class="transform transition-transform duration-300 ease-in-out hover:scale-110 hover:text-green-500 w-8 h-8 flex items-center justify-center">
...
<div id="modalDeleteAlternative" class="flex fixed inset-0 p-4 flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] hidden">
    <div class="w-full max-w-md bg-white dark:bg-slate-800 shadow-lg rounded-md p-6 relative">
        ...
        <div class="flex justify-center space-x-4">
            <button id="modalDeleteAlternativeClose" type="button" class="text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-700 text-sm font-medium hover:text-rose-600 dark:hover:text-rose-600 hover:border-rose-600 dark:hover:border-rose-600 border-l-4 py-2 px-3 rounded">
                Cancel
            </button>
