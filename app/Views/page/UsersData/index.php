...
<div class="px-6">
    <div class="py-4 flex flex-col">
        <span class="text-lg font-bold text-black dark:text-white">Users Data</span>
        <span class="text-sm text-black dark:text-white">Managing and accessing information related to user profiles, permissions, and activities within the system.</span>
    </div>
    <div>
            ...
            <tbody class="text-sm font-thin text-black dark:text-white">
                <?php foreach ($userData as $index => $row) : ?>
                    <tr>
                        ...
                        <td class="border border-gray-200 dark:border-gray-700"><?php echo $row['nama']; ?></td>
                        <td class="border border-gray-200 dark:border-gray-700"><?php echo $row['username']; ?></td>
                        <td class="border border-gray-200 dark:border-gray-700"><?php echo $row['email']; ?></td>
                        <td class="border border-gray-200 dark:border-gray-700 text-rose-600 font-semibold"><?php echo $row['id_user_level']; ?></td>
                        <td>
                            <div class="flex items-center space-x-2 text-lg">
                                <button onclick="showEditUser(<?php echo $row['id']; ?>)" class="transform transition-transform duration-300 ease-in-out hover:scale-110 hover:text-green-500 w-8 h-8 flex items-center justify-center">
...
<div id="modalDeleteUsers" class="flex fixed inset-0 p-4 flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] hidden">
    <div class="w-full max-w-md bg-white dark:bg-slate-800 shadow-lg rounded-md p-6 relative">
        <div class="my-6 text-center">
            ...
            <h4 class="text-xl font-semibold mt-6 text-black dark:text-white">Delete Users?</h4>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-4">You're going to delete this Users. Are you sure?</p>
        </div>
        <div class="flex justify-center space-x-4">
            <button id="modalDeleteUsersClose" type="button" class="text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-700 text-sm font-medium hover:text-rose-600 dark:hover:text-rose-600 hover:border-rose-600 dark:hover:border-rose-600 border-l-4 py-2 px-3 rounded">
                Cancel
            </button>
